...
exports.EphemeralManager = void 0;
...
class EphemeralManager {
    ...
    async enableForChat(jid, timerMinutes = 60) {
        ...
        db.prepare('UPDATE chats SET ephemeral_mode = 1, ephemeral_timer = ?, ephemeral_start_timestamp = ? WHERE instance_id = ? AND jid = ?')
            .run(timerMinutes, now, this.instanceId, (0, utils_1.normalizeJid)(jid));
        ...
        await this.sock.sendMessage((0, utils_1.normalizeJid)(jid), { text: `[System] Ephemeral Mode Enabled. Messages will be auto-cleared from this device after ${timerMinutes} minutes.` });
    }
    async disableForChat(jid) {
        const db = (0, database_1.getDb)();
        db.prepare('UPDATE chats SET ephemeral_mode = 0 WHERE instance_id = ? AND jid = ?')
            .run(this.instanceId, (0, utils_1.normalizeJid)(jid));
        console.log(`[EphemeralManager ${this.instanceId}]: Disabled for ${jid}`);
        this.io.emit('chat_update', { instanceId: this.instanceId });
        await this.sock.sendMessage((0, utils_1.normalizeJid)(jid), { text: `[System] Ephemeral Mode Disabled.` });
    }
    async handleIncomingMessage(jid, text, fromMe) {
        ...
        const db = (0, database_1.getDb)();
        const startEmoji = db.prepare('SELECT value FROM settings WHERE instance_id = ? AND key = ?').get(this.instanceId, 'ephemeral_trigger_start')?.value
            || db.prepare('SELECT value FROM settings WHERE instance_id = 0 AND key = ?').get('ephemeral_trigger_start')?.value
            || '👻';
        const stopEmoji = db.prepare('SELECT value FROM settings WHERE instance_id = ? AND key = ?').get(this.instanceId, 'ephemeral_trigger_stop')?.value
            || db.prepare('SELECT value FROM settings WHERE instance_id = 0 AND key = ?').get('ephemeral_trigger_stop')?.value
            || '🛑';
    ...
    async processCleanup() {
        ...
        const chats = db.prepare('SELECT jid, ephemeral_timer, ephemeral_start_timestamp FROM chats WHERE instance_id = ? AND ephemeral_mode = 1').all(this.instanceId);
        for (const chat of chats) {
            if (!chat.ephemeral_start_timestamp)
                continue;
            ...
            const messages = db.prepare(`
                ...
                AND timestamp > ?
            `).all(this.instanceId, chat.jid, chat.ephemeral_start_timestamp);
            const toDelete = [];
...
exports.EphemeralManager = EphemeralManager;
