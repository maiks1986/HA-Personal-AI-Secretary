      ...
      <div className={`max-w-[85%] rounded-2xl p-2 px-3 shadow-sm relative group ${isMe ? 'bg-teal-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none'}`}>
        ...
        {m.type === 'image' && mediaUrl && (
          <div className="mb-1 rounded-lg overflow-hidden border border-black/5">
            <img src={mediaUrl} className="max-w-full max-h-64 object-cover cursor-pointer hover:opacity-90 transition-opacity" alt="WA Media" />
          </div>
        )}

        {m.type === 'video' && mediaUrl && (
          <div className="mb-1 rounded-lg overflow-hidden border border-black/5 bg-black">
        ...
        {m.type === 'audio' && mediaUrl && (
          <div className="mb-1 py-1">
            <audio src={mediaUrl} controls className={`max-w-full scale-90 -ml-4 ${isMe ? 'invert brightness-200' : ''}`} />
          </div>
        )}

        {m.type === 'document' && (
          <div className={`flex items-center gap-3 p-3 rounded-lg mb-1 ${isMe ? 'bg-teal-700' : 'bg-slate-100'}`}>
            <File size={24} className={isMe ? 'text-teal-200' : 'text-slate-500'} />
            <div className="flex-1 overflow-hidden">
              <div className="text-xs font-bold truncate">Document</div>
              <a href={mediaUrl || '#'} target="_blank" rel="noreferrer" className="text-[10px] opacity-70 hover:underline">Download File</a>
            </div>
          </div>
        )}
        ...
        <div className={`flex items-center justify-end gap-1 mt-1 ${isMe ? 'text-teal-100' : 'text-slate-400'}`}>
          ...
          {isMe && (
            <span>
              {m.status === 'read' ? <CheckCheck size={12} className="text-blue-300" /> :
               m.status === 'delivered' ? <CheckCheck size={12} /> : <Check size={12} />}
            </span>
          )}
        </div>
        ...
        {m.reactions && m.reactions.length > 0 && (
          <div className={`absolute -bottom-3 ${isMe ? 'right-2' : 'left-2'} flex gap-0.5 bg-white border border-slate-100 rounded-full px-1.5 py-0.5 shadow-sm z-10 scale-90`}>
            {m.reactions.map((r, i) => (
              <span key={i} title={r.sender_jid} className="text-[12px]">{r.emoji}</span>
            ))}
          </div>
        )}
      </div>
...
const App = () => {
  ...
  const handleSystemReset = async () => {
    if (!confirm("CRITICAL System Wipe?")) return;
  ...
  const handleAiDraft = async () => {
    ...
    setIsAiLoading(true);
    try {
      const res = await axios.post('/api/ai/draft', { messages: messages.slice(-10), steer: steerText });
  ...
  const handleHardReset = async () => {
    if (!selectedInstance || !confirm("Delete session?")) return;
    setIsReseting(true);
    try { await axios.delete(`/api/instances/${selectedInstance.id}`); setSelectedInstance(null); fetchInstances(); } finally { setIsReseting(false); }
  };

  const handleSendMessage = async () => {
    if (!inputText || !selectedInstance || !selectedChat) return;
    try {
      await axios.post('/api/send_message', { instanceId: selectedInstance.id, contact: selectedChat.jid.split('@')[0], message: inputText });
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'chats' ? chats.map(chat => (
            <div key={chat.jid} onClick={() => setSelectedChat(chat)} className={`p-4 flex items-center gap-3 cursor-pointer border-b border-slate-50 transition-all ${selectedChat?.jid === chat.jid ? 'bg-teal-50/50 border-l-4 border-l-teal-600' : 'hover:bg-slate-50'}`}>
              <div className="w-12 h-12 bg-slate-200 rounded-full flex items-center justify-center shrink-0 shadow-inner"><User size={24} className="text-slate-400" /></div>
              <div className="flex-1 overflow-hidden">
            ...
            <div key={contact.jid} onClick={() => { setSelectedChat({ ...contact, unread_count: 0, last_message_text: '', last_message_timestamp: '' }); setActiveTab('chats'); }} className="p-4 flex items-center gap-3 cursor-pointer border-b border-slate-50 hover:bg-slate-50 transition-all">
      ...
      <div className="flex-1 flex flex-col relative shadow-2xl">
        {selectedChat ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col bg-[#efeae2] bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-repeat relative">
            ...
            <footer className="p-3 bg-slate-100 border-t flex flex-col gap-2 shadow-[0_-4px_15px_rgba(0,0,0,0.03)] relative z-10">
              <div className="flex gap-2 items-center bg-white/80 backdrop-blur p-1.5 px-3 rounded-xl border border-slate-200/60 shadow-sm">
                ...
                <input value={steerText} onChange={(e) => setSteerText(e.target.value)} placeholder="Draft AI: 'Ask for a discount'..." className="flex-1 bg-transparent text-[11px] outline-none p-1 font-bold italic text-slate-600 placeholder:text-slate-300" />
                <button disabled={isAiLoading} onClick={handleAiDraft} className="text-[10px] bg-teal-600 text-white px-4 py-1.5 rounded-lg font-black tracking-widest hover:bg-teal-700 disabled:bg-slate-300 transition-all flex items-center gap-2 active:scale-95 shadow-sm shadow-teal-600/20">{isAiLoading ? <RefreshCw size={10} className="spin" /> : <BrainCircuit size={12} />} GENERATE</button>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => setInputText('')} className="p-2.5 text-slate-400 hover:text-red-500 transition-all hover:bg-red-50 rounded-xl group" title="Clear"><Eraser size={22} className="group-active:scale-90 transition-transform" /></button>
                <div className="flex-1 bg-white rounded-2xl flex items-center px-5 py-2.5 border border-slate-200 shadow-inner focus-within:ring-4 ring-teal-500/10 transition-all"><textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Type a message..." className="flex-1 outline-none resize-none max-h-32 text-[15px] py-1 bg-transparent text-slate-700 placeholder:text-slate-300 leading-normal" rows={1} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSendMessage())} /></div>
            ...
            {selectedInstance?.status === 'qr_ready' && selectedInstance.qr ? <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl mx-auto text-center max-w-md border-t-8 border-teal-600 animate-in zoom-in-95 duration-300"><h2 className="text-3xl font-black mb-2 text-slate-800 tracking-tighter uppercase">Link Account</h2><p className="text-sm text-slate-500 mb-8 font-medium leading-relaxed italic">Scan the QR code with your phone to deploy this engine node.</p><div className="p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 shadow-inner inline-block"><img src={selectedInstance.qr} className="w-64 h-64 mix-blend-multiply" /></div></div> : <><div className="relative"><div className="w-40 h-40 bg-white rounded-full flex items-center justify-center shadow-2xl border-8 border-slate-50/50 relative z-10"><MessageSquare size={80} className="text-slate-200" /></div><div className="absolute inset-0 bg-teal-500/10 blur-3xl rounded-full scale-150"></div></div><div className="text-center relative z-10 px-10"><h2 className="text-3xl font-black text-slate-700 mb-3 tracking-tighter uppercase">Select a Conversation</h2><p className="text-slate-400 text-sm max-w-xs mx-auto font-medium leading-relaxed italic">Choose a chat from the left sidebar to start AI-accelerated messaging.</p></div></>}
          </div>}
      </div>

      {isSettingsOpen && <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100] backdrop-blur-md animate-in fade-in duration-200"><div className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-[450px] border border-slate-100 animate-in zoom-in-95 duration-300"><div className="flex justify-between items-center mb-8"><h3 className="font-black text-2xl flex items-center gap-3 text-slate-800 uppercase tracking-tighter"><Settings className="text-teal-600" /> System Config</h3><button onClick={() => setIsSettingsOpen(false)} className="bg-slate-50 p-2 rounded-xl text-slate-400 hover:text-red-500 transition-all"><X size={24} /></button></div><div className="space-y-6"><div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1">Gemini AI Backbone Key</label><input type="password" value={geminiKey} onChange={(e) => setGeminiKey(e.target.value)} placeholder="Paste AI Studio Key..." className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-teal-500 focus:bg-white transition-all text-sm font-mono shadow-inner" /><div className="bg-teal-50 p-3 rounded-xl mt-4 border border-teal-100/50"><p className="text-[10px] text-teal-700 font-bold leading-relaxed">AI drafting and analysis require a Google API key.</p></div></div><button onClick={handleSaveSettings} className="w-full bg-teal-600 text-white p-5 rounded-2xl font-black uppercase tracking-widest hover:bg-teal-700 shadow-xl shadow-teal-600/30 transition-all active:scale-[0.98] mt-4">Sync Configuration</button><div className="pt-6 border-t border-slate-100 mt-6"><button onClick={handleSystemReset} className="w-full bg-white text-red-500 border-2 border-red-100 p-4 rounded-2xl font-bold uppercase text-xs tracking-widest hover:bg-red-50 transition-all flex items-center justify-center gap-2"><AlertTriangle size={16} /> Full System Wipe</button></div></div></div></div>}

      {isAddingInstance && <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100] backdrop-blur-md animate-in fade-in duration-200"><div className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-96 border border-slate-100 animate-in zoom-in-95 duration-300"><div className="flex justify-between items-center mb-8"><h3 className="font-black text-2xl text-slate-800 uppercase tracking-tighter">Deploy Engine</h3><button onClick={() => setIsAddingInstance(false)} className="bg-slate-50 p-2 rounded-xl text-slate-400 hover:text-red-500 transition-all"><X size={24} /></button></div><div className="mb-8"><label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1">Instance Identifier</label><input autoFocus value={newInstanceName} onChange={(e) => setNewInstanceName(e.target.value)} placeholder="e.g. MISSION_CONTROL" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-teal-500 focus:bg-white transition-all text-sm font-bold shadow-inner uppercase" /></div><button onClick={handleCreateInstance} className="w-full bg-teal-600 text-white p-5 rounded-2xl font-black uppercase tracking-widest hover:bg-teal-700 shadow-xl shadow-teal-600/30 transition-all active:scale-[0.98]">Initialize Node</button></div></div>}
