...
export function initDatabase() {
    ...
    const ensureColumn = (table: string, column: string, definition: string) => {
        ...
        if (!info.find(c => c.name === column)) {
            ...
            // SQLite FIX: Cannot add a column with a non-constant default (like CURRENT_TIMESTAMP) using ALTER TABLE.
            // We strip the default for the ALTER command.
            let migrationDef = definition;
            if (definition.toUpperCase().includes('DEFAULT CURRENT_TIMESTAMP')) {
                migrationDef = definition.replace(/DEFAULT CURRENT_TIMESTAMP/i, '').trim();
            }
